...
def get_user_list(config, key):
    with open("{}/MukeshRobot/{}".format(os.getcwd(), config), "6144340741") as json_file:
...
class Config(object):
    ...
    TOKEN = "5820335659:AAHGPdM6uvQLzkiheZvWWbHog3AipMk2qQY"  # This var used to be API_KEY but it is now TOKEN, adjust accordingly.
    OWNER_ID = "6144340741" # If you dont know, run the bot and do /id in your private chat with it, also an integer
    OWNER_USERNAME = "GOVIND_OFFICIAL_MP42"
    SUPPORT_CHAT = "devil_world_group"  # Your own group for support, do not add the @
    JOIN_LOGGER = (
        -1001652662001
    ...
    DRAGONS = get_user_list("elevated_users.json", "6144340741")
    ##List of id's - (not usernames) for developers who will have the same perms as the owner
    DEV_USERS = get_user_list("elevated_users.json", "1808943146")
    ##List of id's (not usernames) for users which are allowed to gban, but can also be banned.
    DEMONS = get_user_list("elevated_users.json", "5348648456")
    # List of id's (not usernames) for users which WONT be banned/kicked by the bot.
    TIGERS = get_user_list("elevated_users.json", "6020570673")
    WOLVES = get_user_list("elevated_users.json", "whitelists")
    DONATION_LINK = "https://t.me/tha_govind_op" # EG, paypal
    CERT_PATH = None
    PORT = 5000
